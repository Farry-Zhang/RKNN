% 二维RKNN网络训练函数
function [trained_net, training_losses] = trainRKNN_2d_new(net, param)
    % 训练参数
    num_epochs = 200;
    learning_rate = 0.005;
    window_size = 5;
    ...
    % Kalman初始化函数 - 二维6状态 [px py vx vy ax ay]
    initialize_kalman = @() deal(zeros(6,1), diag([10, 10, 1, 1, 1, 1]));
    ...
    for epoch = 1:num_epochs
        ...
        [train_inputs, train_targets] = get_rknn_training_data_2d_new(batch_size, param);
        
        epoch_loss = 0;
        
        % 初始化梯度累积器
        hidden_dim = size(net.Linear1_weight, 1);
        total_grad_L1_w = zeros(hidden_dim, 8);
        total_grad_L1_b = zeros(hidden_dim, 1);
        total_grad_L2_w = zeros(hidden_dim, 2);
        total_grad_L2_b = zeros(hidden_dim, 1);
        total_grad_L3_w = zeros(4, hidden_dim);
        total_grad_L3_b = zeros(4, 1);
        
        total_windows = 0;
        
        % 添加测量噪声
        noisy_inputs = add_measurement_noise_2d(train_inputs, sqrt(param.r0_x + param.r0_y)/2);
        ...
        for batch = 1:batch_size
            ...
            prev_z = zeros(2, 1);
             ...
             for k = 1:10               
                true_pos = squeeze(train_targets(batch, k, :)); % 2×1，确保是列向量
                true_pos = true_pos(:); % 强制转换为列向量，此时是2*1的列向量
                % 添加观测噪声
                noisy_z = true_pos + [sqrt(param.r0_x); sqrt(param.r0_y)] .* randn(2,1);
                
                % 使用固定参数进行Kalman更新
                fixed_output = ones(4, 1); % αx=1, αy=1, βx=1, βy=1
                [x_est, P, kalman_cache] = kalman_update_2d_new(x_est, P, fixed_output, noisy_z, param, true_pos); % 添加 true_pos 参数
                                
                % 计算位置误差
                pos_error = norm(true_pos - x_est(1:2)); % 欧氏距离误差L2范数
                
                % 缓存数据（前10步不用于RKNN训练，但记录误差）
                cache(k).input = zeros(10, 1); % 空输入
            ...
            for k = 11:trajectory_length
                true_pos = squeeze(train_targets(batch, k, :)); % 2×1
                % 添加观测噪声
                noisy_z = true_pos + [sqrt(param.r0_x); sqrt(param.r0_y)] .* randn(2,1);
                ...
                x_pred = A * x_est;  % 6×1，提前计算，保证在 k==11 分支也存在
                               
                % 获取当前输入（前一步的预测结果）
                if k == 11
                    % 第11步使用预先生成的输入（已经是10×1）
                    current_input = squeeze(noisy_inputs(batch, k, :)); 
                else
                    % ---- 状态增量归一化 ----
                    x_pred_prev = cache(k-1).kalman_cache.x_pred;  % 取前一时刻预测（第11步时不会用到）
                    x_pred_norm = (x_pred - x_pred_prev) / param.max_velocity; % 6×1
                    
                    % 计算新息向量
                    v = noisy_z - H * x_pred; % 2×1
                    
                    % 计算新息协方差矩阵S
                    P_pred = A * P * A' + blkdiag(param.Q0_x, param.Q0_y); % 6×6
                    S = H * P_pred * H' + diag([param.r0_x, param.r0_y]); % 2×2
                ...
                % RKNN前向传播
                [output, hidden] = runRKNN_2d_new(net, current_input);
                
                % Kalman更新（kalman_update_2d_new 应该返回更新后的 kalman_cache）
                [x_est, P, kalman_cache] = kalman_update_2d_new(x_est, P, output, noisy_z, param, true_pos); % 添加 true_pos 参数
                
                % 把当前步的预测状态存入 kalman_cache 方便下次用（保证下一步能取到 cache(k).kalman_cache.x_pred）
                kalman_cache.x_pred = x_pred;
                
                % 计算位置误差
                pos_error = norm(true_pos - x_est(1:2)); % 欧氏距离误差
            ...
            for k = 16:trajectory_length
                ...
                if window_start >= 11
                    ...
                    [grad_L1_w, grad_L1_b, grad_L2_w, grad_L2_b, grad_L3_w, grad_L3_b] = ...
                        rknn_backward_2d_new(cache(window_start:window_end), window_size, param, net);
